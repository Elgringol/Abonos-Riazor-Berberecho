...
const Dashboard: React.FC = () => {
  ...
  // Referencia para tener siempre el estado más reciente para persistencia
  ...
  }, [assignments, matchHistory, cycleHistory, lastResetTime, matchName, raffleWinners, winnersStatus, reserveList, reserveWinners, wasCycleResetInCurrentRaffle]);
  ...
  const refreshAllData = async () => {
      ...
      try {
          ...
          if (cloudState && !isCloudEmpty) {
              ...
              const hasCloudActiveRaffle = cloudState.activeRaffle !== undefined;
              const activeRaffleToLoad = hasCloudActiveRaffle ? cloudState.activeRaffle : (() => {
                  const savedActiveRaffle = localStorage.getItem('active_raffle_v2');
                  return savedActiveRaffle ? JSON.parse(savedActiveRaffle) : null;
              })();

              if (activeRaffleToLoad) {
                  setMatchName(activeRaffleToLoad.matchName || '');
                  setRaffleWinners(activeRaffleToLoad.winners || []);
                  setWinnersStatus(activeRaffleToLoad.winnersStatus as Record<string, WinnerStatus> || {});
                  setReserveList(activeRaffleToLoad.reserveList || []);
                  setReserveWinners(activeRaffleToLoad.reserveWinners || []);
                  setWasCycleResetInCurrentRaffle(!!activeRaffleToLoad.isCycleReset);
              } else {
              ...
              // 3. Fallback a Local Storage si falla la nube o no está configurada
              ...
              const savedActiveRaffle = localStorage.getItem('active_raffle_v2');
              if (savedActiveRaffle) {
                  const active = JSON.parse(savedActiveRaffle);
                  setMatchName(active.matchName || '');
                  setRaffleWinners(active.winners || []);
                  setWinnersStatus(active.winnersStatus || {});
                  setReserveList(active.reserveList || []);
                  setReserveWinners(active.reserveWinners || []);
                  setWasCycleResetInCurrentRaffle(!!active.isCycleReset);
              }
              
              setSyncStatus(isConfigured ? 'error' : 'idle');
              ...
              if (isConfigured && isCloudEmpty) {
                  ...
                  if (hasLocalData) {
                      const migrationState: AppState = { 
                          ...
                          cycleHistory: cleanCycleHistory, 
                          lastResetTime: localLastResetTime, 
                          activeRaffle: savedActiveRaffle ? JSON.parse(savedActiveRaffle) : null 
                      };
  ...
  const lastSavePromise = React.useRef<Promise<boolean>>(Promise.resolve(true));

  const persistState = async (overrides: Partial<AppState> = {}) => {
      // Encadenamos el guardado para que sean secuenciales
      const p = (async () => {
          await lastSavePromise.current.catch(() => {}); // Esperamos al anterior aunque falle
          
          setSyncStatus('syncing');
          
          // Combinamos el estado actual (ref para asegurar datos frescos) con los overrides
          const base = stateRef.current;
          const currentState: AppState = { 
              assignments: overrides.assignments !== undefined ? overrides.assignments : base.assignments, 
              matchHistory: overrides.matchHistory !== undefined ? overrides.matchHistory : base.matchHistory, 
              cycleHistory: overrides.cycleHistory !== undefined ? overrides.cycleHistory : base.cycleHistory, 
              lastResetTime: overrides.lastResetTime !== undefined ? overrides.lastResetTime : base.lastResetTime, 
              activeRaffle: overrides.activeRaffle !== undefined ? overrides.activeRaffle : (base.activeRaffle.matchName || base.activeRaffle.winners.length > 0 ? {
                  ...base.activeRaffle,
                  timestamp: Date.now()
              } : null),
          };

          // Guardar Local (Backup crítico inmediato)
          try {
              ...
              if (currentState.activeRaffle) {
                  localStorage.setItem('active_raffle_v2', JSON.stringify(currentState.activeRaffle));
              } else {
                  localStorage.removeItem('active_raffle_v2');
              }
          } catch (e) {
  ...
  const transferWinnersToSlots = async () => {
      ...
      await persistState({ assignments: newAssignments, lastResetTime: now });
      alert(`✅ ASIGNACIÓN COMPLETADA`);
  };
  
  const handleCloseMatchday = async () => {
      ...
      // Guardar el estado histórico ANTES de limpiar el local
      const saveSuccess = await persistState({ 
      ...
      if (saveSuccess) {
          // Limpiar estado del sorteo solo si se guardó correctamente
          setMatchName(''); 
          setRaffleWinners([]); 
          setReserveList([]); 
          setReserveWinners([]); 
  ...
  const handleAddReserve = async (member: Member) => {
      if (!reserveList.find(m => m.id === member.id)) {
          ...
          await persistState({ activeRaffle: { matchName, winners: raffleWinners, winnersStatus, reserveWinners, timestamp: Date.now(), reserveList: newList, isCycleReset: wasCycleResetInCurrentRaffle } });
      }
  };
  ...
  const handleAssignMember = async (member: Member) => {
      if (currentSlotId !== null) {
          ...
          await persistState({ assignments: newAssignments });
      }
  };
